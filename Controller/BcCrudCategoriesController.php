<?php
class BcCrudCategoriesController extends BcCrudAppController {
	...
	public function admin_index($bcCrudContentId) {
		...
		$this->pageTitle = sprintf(__d('baser', '%s｜カテゴリ一覧'), strip_tags($this->request->params['Content']['title']));
		$this->search = 'bc_crud_categories_index';
		$this->help = 'bc_crud_categories_index';
	}
	...
	public function admin_add($bcCrudContentId) {
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if ($this->{$this->modelClass}->saveAll($this->request->data)) {
				...
				$this->BcMessage->setSuccess(sprintf(__d('baser', 'カテゴリ「%s」を追加しました。'), $this->request->data[$this->modelClass]['title']));
	...
	public function admin_edit($bcCrudContentId, $id) {
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if ($this->{$this->modelClass}->saveAll($this->request->data)) {
				clearViewCache();
				$this->BcMessage->setSuccess(sprintf(__d('baser', 'カテゴリ「%s」を更新しました。'), $this->request->data[$this->modelClass]['title']));
		...
		if ($this->request->data[$this->modelClass]['status']) {
			$this->set(
				'publishLink',
				$this->Content->getUrl(
					urldecode($this->request->params['Content']['url']) . 'category/' . $this->request->data[$this->modelClass]['no'],
					true,
