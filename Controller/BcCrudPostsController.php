<?php
class BcCrudPostsController extends BcCrudAppController {
	...
	public function admin_index($bcCrudContentId) {
		...
		$this->{$this->modelClass}->reduceAssociations(['BcCrudContent', 'BcCrudCategory', 'BcCrudTag']);
		$this->{$this->modelClass}->BcCrudContent->reduceAssociations([]);
		$this->{$this->modelClass}->BcCrudCategory->reduceAssociations([]);
		$this->{$this->modelClass}->BcCrudTag->reduceAssociations([]);
		...
		$this->pageTitle = sprintf(__d('baser', '%s｜一覧'), strip_tags($this->request->params['Content']['title']));
		$this->search = 'bc_crud_posts_index';
		$this->help = 'bc_crud_posts_index';
	}
	...
	protected function _setAdminIndexViewData($bcCrudContentId) {
		...
		$this->set('bcCrudCategories', $this->BcCrudCategory->getList($bcCrudContentId));
		$this->set('bcCrudTags', $this->BcCrudTag->getList($bcCrudContentId));
	...
	protected function _createAdminIndexConditions($bcCrudContentId, $data) {
		...
		if (isset($data[$this->modelClass]['name'])) {
			$conditions[$this->modelClass . '.name LIKE'] = '%' . h(trim($data[$this->modelClass]['name'])) . '%';
		}

		if (isset($data[$this->modelClass]['bc_crud_category_id'])) {
			$conditions[$this->modelClass . '.bc_crud_category_id'] = $data[$this->modelClass]['bc_crud_category_id'];
		}

		if (isset($data[$this->modelClass]['bc_crud_tag_id'])) {
			// TODO : tag
		}
	...
	public function admin_add($bcCrudContentId) {
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if (!empty($this->request->data[$this->modelClass]['posts_date'])) {
				$this->request->data[$this->modelClass]['posts_date'] = str_replace('/', '-', $this->request->data[$this->modelClass]['posts_date']);
			}
			...
			// データを保存
			$this->{$this->modelClass}->hasAndBelongsToMany['BcCrudTag']['conditions'] = [];
			if ($this->{$this->modelClass}->saveAll($this->request->data)) {
				clearViewCache();
				$id = $this->{$this->modelClass}->getLastInsertId();
				$this->BcMessage->setSuccess(sprintf(__d('baser', '「%s」を追加しました。'), $this->request->data[$this->modelClass]['name']));
				$this->{$this->modelClass}->recursive = 1;

				// EVENT BcCrudPosts.afterAdd
	...
	public function admin_edit($bcCrudContentId, $id) {
		...
		if ($this->request->is(['post', 'put'])) {
			...
			if ($this->{$this->modelClass}->saveAll($this->request->data)) {
				clearViewCache();
				$this->BcMessage->setSuccess(sprintf(__d('baser', '「%s」を更新しました。'), $this->request->data[$this->modelClass]['name']));

				// EVENT BcCrudPosts.afterEdit
		...
		if ($this->request->data[$this->modelClass]['status']) {
			$this->set(
				'publishLink',
				$this->Content->getUrl(
					urldecode($this->request->params['Content']['url']) . 'detail/' . $this->request->data[$this->modelClass]['no'],
					true,
					$this->request->params['Site']['use_subdomain']
				)
			);
